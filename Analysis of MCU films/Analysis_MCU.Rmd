...
ggplot(data=merged, aes(x=Year)) + 
  ...
  ylab(label="Dollars") +
  scale_colour_manual(name="", values=c(Gross_MovingAvg="#00BA38", Budget_MovingAvg="red", Gross_Loess="#619CFF"))
```
...
ggplot(merged, aes(x=Revenue, y=Budget)) +
  ...
  geom_line(aes(x=Revenue,y=rollapplyr(merged$Budget,width=3,FUN=mean,partial=T),colour="Moving_Average"),size=.6 ) +
  ylab(label="Budget (Dollars)") +
  xlab(label="Revenue (Dollars)") +
  labs(title="Budget vs Revenue for MCU movies") +
  scale_colour_manual(name="", values=c(Moving_Average="#619CFF", Loess="#00ba38"))
```
